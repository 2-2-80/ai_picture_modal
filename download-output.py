...
volume = modal.NetworkFileSystem.new().persist(volume_key)
...
def download_image_using_modal(image_path: str):
  ...
  subprocess.run(f'modal nfs get {volume_key} {os.path.join(remote_outputs_dir, image_path)} {download_dest}'.replace("\\", "/"), shell=True)
